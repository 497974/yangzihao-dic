...
/**
 * Frozen v100 fixtures. Written out in full on purpose: an example file that
 * spreads the previous version is not a snapshot, because editing v099 would
 * silently change what v100 asserts.
 *
 * Delta from v099: `videoSubtitles.style` gains `customCSS`, the extra CSS the
 * subtitle style page can layer on top of the picked fonts and colours. Every
 * existing profile arrives with it off, so every series here carries `null`.
 */
