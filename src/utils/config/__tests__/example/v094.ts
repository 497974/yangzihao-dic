...
/**
 * Frozen v094 fixtures. Written out in full on purpose: an example file that
 * spreads the previous version is not a snapshot, because editing v093 would
 * silently change what v094 asserts.
 *
 * Delta from v093: every fixture gains `translationHub.shortcut`, the new
 * key combination that opens the Translation Hub page. None of these fixtures
 * had "Alt+Shift+H" bound elsewhere, so they all take the primary default;
 * the collision fallback is covered by
 * `migration-scripts/v093-to-v094.test.ts`.
 */
