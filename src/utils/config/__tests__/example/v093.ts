...
/**
 * Frozen v093 fixtures. Written out in full on purpose: an example file that
 * spreads the previous version is not a snapshot, because editing v092 would
 * silently change what v093 asserts.
 *
 * Delta from v092: none of these fixtures pairs translationOnly page mode
 * with a Microsoft page-translate provider, so the v092-to-v093 rewrite
 * (Microsoft loses translationOnly support) leaves them unchanged. The
 * rewrite and its fallbacks are covered by
 * `migration-scripts/v092-to-v093.test.ts`.
 */
