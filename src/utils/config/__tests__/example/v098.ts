...
/**
 * Frozen v098 fixtures. Written out in full on purpose: an example file that
 * spreads the previous version is not a snapshot, because editing v097 would
 * silently change what v098 asserts.
 *
 * Delta from v097: every series gains the Jalapeno Cloud provider at the head of
 * its provider list. No headers on it — Jalapeno's attribution headers are forced
 * at request time and deliberately never enter stored config.
 */
