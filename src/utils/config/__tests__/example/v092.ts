...
/**
 * Frozen v092 fixtures. Written out in full on purpose: an example file that
 * spreads the previous version is not a snapshot, because editing v091 would
 * silently change what v092 asserts.
 *
 * Delta from v091: legacy null prompt selections become the real built-in id
 * `default`. None of these fixtures has a custom prompt whose id collides with
 * a new built-in; deterministic collision handling and selected-id updates are
 * covered by `migration-scripts/v091-to-v092.test.ts`.
 */
