...
/**
 * Frozen v095 fixtures. Written out in full on purpose: an example file that
 * spreads the previous version is not a snapshot, because editing v094 would
 * silently change what v095 asserts.
 *
 * Delta from v094: the top-level `translate` section is renamed to
 * `pageTranslation` so the feature identifier matches the hosted AI contract.
 * `selectionToolbar.features.translate` is a different surface and keeps its
 * name; edge cases live in `migration-scripts/v094-to-v095.test.ts`.
 */
